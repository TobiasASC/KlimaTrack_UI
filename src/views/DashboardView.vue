    ...
    <div class="row">
        ...
        <div class="card text-bg-dark h-100">
          ...
          <div class="card-body d-flex flex-column justify-content-center pt-0">
            ...
            <div class="d-flex align-items-center mb-3">
              <!-- Ícono de clima -->
              <i
                class="bi bi-cloud-moon text-info"
                style="font-size: 5.5rem; line-height: 1; margin-right: 20px"
              ></i>
              <!-- Temperatura -->
              <h1 class="display-1 fw-bold mb-0">{{ lecturaActual?.temperatura }}°C</h1>
            </div>

            <p class="card-text fs-5 text-light opacity-75">
              {{ lecturaActual?.humedad }}% &nbsp;|&nbsp; Viento: {{ lecturaActual?.viento }} km/h
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="col-md-6 mb-4 texto_secundario">
        <div class="card text-bg-dark">
          <div class="card-header">Tendencia Dual: Temperatura y Humedad</div>
          <div class="card-body">
      ...
      <div class="col-md-8 mb-4 texto_secundario">
        <div class="card text-bg-dark">
          <div class="card-header">Precipitaciones Acumuladas</div>
          <apexchart
            type="bar"
            height="160"
      ...
      <div class="col-md-8 mb-4 texto_secundario">
        <div class="card text-bg-dark">
          <div class="card-header">Mapa de alertas</div>
          <apexchart
            type="heatmap"
            height="160"
...
// Variables que se usaran en el dashboard
const ubicacionId = ref(1)

const cargando = ref(true)
const error = ref(null)

// Datos de la tarjeta resumen (lectura actual)
const lecturaActual = ref(null)

const cargarLecturaActual = async () => {
  try {
    lecturaActual.value = await climaService.ultimaLectura(ubicacionId.value)
  } catch (e) {
    error.value = 'No se pudo obtener la lectura actual'
    console.error(e)
  }
}
